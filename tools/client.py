...
def store(url: str, file: str):
    ...
    return response.json() if response.status_code == 200 else "Could not store {}".format(file)
...
def find(url: str, id: str):
    ...
    try:
        ...
        print("Status={}".format(response.status_code), file=sys.stderr)
        return response.json() if response.status_code == 200 else "File {} not found".format(id)
    except:
        return "{} is an invalid ID".format(id)
